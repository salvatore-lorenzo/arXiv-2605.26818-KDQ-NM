"""Reproduce Fig. 3: an off-resonant, non-energy-preserving collision model.

Same layout as Fig. 1, but detuned: omega_S = 0.8 while omega_M = omega_E =
1.0 (system-memory detuning Delta_omega = -0.2), which breaks energy
conservation in the S-M collision. Two stacked panels, both versus collision
number n:
  1. The three complete-positivity condition tests a_n - 1, b_n, and
     |c_n|^2 - a_n(1 - b_n).
  2. The RHP witness g_n, the KDQ non-positivity functional N_q[P_n], and
...
# Same as Fig. 1's parameters, except omega_s is detuned away from
# resonance (Delta_omega = omega_s - omega_m = -0.2).
...
PLOT_COLLISIONS = 175
OUTPUT_PATH = Path(__file__).with_name("fig_3.pdf")
SAVE_FIGURE = True
SHOW_FIGURE = False
...
class Fig3Results:
    """Container for the quantities plotted in Fig. 3."""
...
def simulate_fig3(params: CollisionParameters = PARAMS) -> Fig3Results:
    """Run the collision model and compute the Fig. 3 diagnostics."""
    ...
    # reconstruction above: a, b, c and the CP-divisibility witnesses need
    # the linear map Lambda_{n,n-1} itself, while N_q needs the actual
    # pre-collision populations of rho_S.
...
def plot_fig3(results: Fig3Results, params: CollisionParameters = PARAMS) -> plt.Figure:
    """Lay out the two stacked panels of Fig. 3 (see module docstring)."""
    ...
    ax = axes[0]
    handles = [
        styled_line(ax, n, results.condition_a[:n_plot], COLORS["green"], "circle", r"$a_n {-} 1$"),
...
def main() -> None:
    results = simulate_fig3(PARAMS)
    fig = plot_fig3(results, PARAMS)
